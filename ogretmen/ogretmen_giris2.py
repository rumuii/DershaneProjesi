...
from Mavi_Dershanem.login_gecis.yukleme2 import SplashScreen
import sqlite3
import sys
...
"border-radius: 12px;")
    ...
    def retranslateUi(self, Form):
        ...
        self.label.setText(_translate("Form", "ÖĞRETMEN"))
        self.email.setPlaceholderText(_translate("Form", "E-mail :"))
        self.parola.setPlaceholderText(_translate("Form", "Branş No :"))
    ...
    def loginfunc(self):
        ...
        if len(user_name) == 0 or len(password_) == 0:
            ...
            try:
                ...
                cur = conn.cursor()
                cur.execute("SELECT brans FROM tbl_ogretmen WHERE email=?", [user_name])
                data = cur.fetchone()[0]
            except TypeError:
                print("Böyle bir Öğretmen bulunmuyor.")
            else:
                if data == password_:
                     self.yukleme4()

                else:
                    print("Brans No yanlış.")
            finally:
                conn.close()
